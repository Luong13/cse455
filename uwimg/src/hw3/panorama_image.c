...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for (int i = 0; i < an; i++) {
        if (seen[m[i].bi]) {
            // match already seen, throw out the match and shift
            // the rest forward by one
...
point project_point(matrix H, point p)
{
    ...
    q.x = x.data[0][0] / x.data[2][0];
    q.y = x.data[1][0] / x.data[2][0];
    return q;
}
...
float point_distance(point p, point q)
{
    ...
    return sqrtf(powf(p.x - q.x, 2.0) + powf(p.y - q.y, 2.0));
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++) {
        point pt = project_point(H, m[count].p);
        if (point_distance(pt, m[count].q) < thresh) {
            count++;
        } else {
            match temp = m[count];
...
matrix compute_homography(match *matches, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        memcpy(M.data[2*i], arr1, sizeof(arr1));
        memcpy(M.data[2*i + 1], arr2, sizeof(arr2));
        b.data[2*i][0] = xp;
        b.data[2*i + 1][0] = yp;
    }
    ...
    for (int k = 0; k < a.c; k++) {
        for (int j = topleft.y; j < botright.y; j++) {
            for (int i = topleft.x; i < botright.x; i++) {
                ...
                if (0 <= pt.x && pt.x < b.w && 0 <= pt.y && pt.y < b.h) {
                    set_pixel(c, k, j - dy, i - dx, bilinear_interpolate(b, k, pt.y, pt.x));
                }
            }
        }
    }
