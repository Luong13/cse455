...
image structure_matrix(image im, float sigma)
{
    ...
    for (int i = 0; i < im.h; i++) {
        for (int j = 0; j < im.w; j++) {
            set_pixel(S, 0, i, j, get_pixel(ix, 0, i, j) * get_pixel(ix, 0, i, j));
            set_pixel(S, 1, i, j, get_pixel(iy, 0, i, j) * get_pixel(iy, 0, i, j));
            set_pixel(S, 2, i, j, get_pixel(ix, 0, i, j) * get_pixel(iy, 0, i, j));
        }
    }
...
image cornerness_response(image S)
{
    ...
    float det, trace, ixx, iyy, ixy;
    static float alpha = 0.06;

    for (int i = 0; i < S.h; i++) {
        for (int j = 0; j < S.w; j++) {
            ixx = get_pixel(S, 0, i, j);
            iyy = get_pixel(S, 1, i, j);
            ixy = get_pixel(S, 2, i, j);

            det = ixx*iyy - ixy*ixy;
            trace = ixx + iyy;

            set_pixel(R, 0, i, j, det - alpha * trace * trace);
        }
    }
    return R;
}
...
image nms_image(image im, int w)
{
    ...
    static float LOW_VALUE = -999999;
    image r = make_image(im.c, im.h, im.w);

    for (int i = 0; i < im.h; i++) {
        for (int j = 0; j < im.w; j++) {
            float q = get_pixel(im, 0, i, j);
            set_pixel(r, 0, i, j, q);
            for (int y = -w; y < w+1; y++) {
                for (int x = -w; x < w+1; x++) {
                    if (q < get_pixel(im, 0, i+y, j+x)) {
                        set_pixel(r, 0, i, j, LOW_VALUE);
                    }
                }
            }
        }
...
descriptor *harris_corner_detector(image im, float sigma, float thresh, int nms, int *n)
{
    ...
    for (int i = 0; i < Rnms.h; i++) {
        for (int j = 0; j < Rnms.w; j++) {
            if (get_pixel(Rnms, 0, i, j) > thresh) {
                count++;
            }
        }
    }
